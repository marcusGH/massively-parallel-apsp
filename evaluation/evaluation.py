...
def set_size(width=455.24411, fraction=1.0):
...
def plot_with_errorbars(ax, ns, ys, err, label, plot_kwargs=None):
    """
    ...
    err = np.array(err)
    ys = np.array(ys)
    # _, caps, bars = ax.errorbar(ns, ys, yerr=err, label=label, elinewidth=5, capsize=7.0, fmt='o', ls='-', **plot_kwargs)
...
def plot_scaling(base_path, ns, ps, y_func, y_err_func):
    ...
    for p in reversed(ps):
        ...
        plot_with_errorbars(ax, ns, ys, err, label=fr"${p} \times {p}$")
    ...
    for n in ns:
        ...
        ys.append(y_func(timings))
        err.append(y_err_func(timings))
    plot_with_errorbars(ax, ns, ys, err, label="Serial", plot_kwargs={'c': 'k'})
    return fig, ax
