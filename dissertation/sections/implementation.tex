...
\end{adjustbox}
\end{center}
\caption[Repository overview]{The directory structure of the code repository. Text in italics
    ...
    directory is presented in \cref{fig:unit-test} in the Evaluation chapter.
    The whole codebase, as presented here, contains 5877 lines of Java code,
    and 728 lines of Python code. The \texttt{work/}, \texttt{memoryModel/}
...
specifying the graphs size, and it has previously
...
To allow the evaluation to better analyse the practicality of using
\ac{MatSquare} for route-planning problems, we want the input graphs
to have matching characteristics to real-world graphs. With a clever choice of
...
\cref{fig:parallel-system-overview}, is designed with a simple
\texttt{Worker} interface in mind.
...
However, the work management is not so simple because
the \acp{PE} interact during communication. As we saw in the previous
...
workers through each phase before moving onto the next, as visualised in
...
associated with
Java's \texttt{Thread} API. Another benefit is implicit synchronisation.
...
\end{center}
\caption[Execution order when managing worker phases]{Example of multithreading when simulating a 4 processing elements on
...
value, but instead modifies the recipient's memory at label \texttt{label},
...
As we will discuss in the following subsection, this gives many benefits.
...
\begin{wrapfigure}{r}{.5\textwidth}
    ...
    \vspace{-10pt}
    \caption[Demonstration of communication model through a simple example]{An example of the communication model based on
    \cref{eq:communication}. The ``$\delta t$''s are the times it takes to
...
$T^{(0)}_{(i,j)}$.
These entries are then updates whenever we advance from phase $(n)$ to phase
...
we use the formula in \autoref{eq:communication}, where the sets
...
for finding the distance- and predecessor matrix, I will look aside from how
the matrix-multiplication procedure is implemented, only focusing on
its desired semantics. Afterwards, I describe a parallel implementation of the
procedure, emphasising how it fulfils the semantics assumed.
...
\begin{definition}
    ...
    \rightsquigarrow j$ in graph $G=(V,E,w)$.
\end{definition}
...
just like in \autoref{alg:matsquare}.
...
the submatrices of $B$ ared shifted upwards to the next \ac{PE}. This pattern
...
\autoref{fig:compression-diagram}.
...
\begin{enumerate}
    \item Nodes $p$ and $q$ are on the same compressed chain, in which case
        ...
        the list of vertices associated with the compressed chain.
        This case corresponds to $A=C$ and $B=D$ in
