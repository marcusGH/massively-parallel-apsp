...
\texttt{MatSquareTest} in \cref{fig:unit-test-full}. This was  done for
both the regular and generalised version of the algorithm.

% \begin{figure}
% \begin{center}
%     \includegraphics[scale=.6]{figs/unit-test-small2.png}
% \end{center}
% \caption[Screenshot of unit tests' sucess]{A screenshot showing that all the unit tests written passed successfully. See \cref{fig:unit-test-full} for the list of each individual test. Additionally, the repository layout for \texttt{test/java/} is found in \cref{sec:Unit tests}}
% \label{fig:unit-test}
% \end{figure}
...
test in \cref{fig:unit-test-full}, and found that it could correctly reconstruct the
...
\begin{figure}[t]
    \begin{center}
        \includegraphics[scale=.6]{figs/unit-test-full.png}
    \end{center}
    \caption[All unit tests passed]{A screenshot showing all of the unit tests written and their status. The structure of the tests correspond to the directory
        structure of \texttt{ParallelAPSP/src/test/java/}, but the full directory
    structure can be found in \cref{sec:Unit tests}.}
    \label{fig:unit-test-full}
\end{figure}
