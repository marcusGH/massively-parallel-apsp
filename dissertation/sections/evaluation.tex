...
built, and the optimisations applied.
...
as shown in lines 7--8 in \cref{alg:matsquare}. To assert the correctness of the
...
\texttt{RepeatedMatrixSquaringTest}s in \cref{fig:unit-test}. They were done for
...
sending a packet across a single interconnect channel, and possibly an uncontested
...
time measures  reduce the amount of noise to give better measurements.
Additionally, a \ac{MIMD} model is used for the communication time, which is more
complicated to implement compared to a na{\"i}ve \ac{SIMD} model, but with
...
I inspected this using \texttt{htop}},
showing the parallelisation was successful.
...
\autoref{tab:communication-constants}.
...
In the ring interconnect used to implement
...
because messages-passing is one-directional in my architecture assumptions
...
the total execution time does not increase, or increases step-wise,
 for small problem sizes, especially for large \ac{PE} layouts.
...
$n \leq 700$, but we can expect other arrangements to also get to this point
with larger problems. Also, as we reach the point where computation start to
dominate, we total execution time starts following a trend  similar to that
...
arrangements for very large problem sizes.
...
The parallel system simulation was showed to work well through unit tests, and
the interface it provides was very easy to work with. I demonstrated the correctness of the main algorithm of the project, and used timing measurements to prove the benefits of parallel computation for solving \ac{APSP} on real-world-like graphs on a wide range of different parallel systems.
